#!/bin/bash
#Time: 2018年3月10日11:39:10
#Author: 十一; modify by: Jmvoid
#GitHub版
...
EOF
	...
	echo "# Author: marisn; Updater:Jmvoid  Ssrpanel:ssrpanel                #"
	echo "# Github: https://github.com/JMVoid/ssrpanel-oneclick-install-centos7 #"
	echo "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"
}
...
function change_password(){
	...
	echo -e "\033[31mThe port you set is:${Change_port:-"3306"}\033[0m"
	
	read -p "Please enter the mysql database port to security:" Change_port
...
EOF
    ...
    if [ ! -f "$myFile" ]; then  
		...
		sed -i "5,10s#3306#${Change_port}#" /etc/my.cnf
		sed -i "s/'port' => '3306'/'port' => '"${Change_port}"'/g" /home/wwwroot/default/config/database.php
...
function install_ssr(){
	...
	./setup_cymysql.sh
	./initcfg.sh
	wget -N -P /root/shadowsocksr/ https://raw.githubusercontent.com/JMVoid/ssrpanel-oneclick-install-centos7/master/user-config.json
	wget -N -P /root/shadowsocksr/ https://raw.githubusercontent.com/JMVoid/ssrpanel-oneclick-install-centos7/master/userapiconfig.py
	wget -N -P /root/shadowsocksr/ https://raw.githubusercontent.com/JMVoid/ssrpanel-oneclick-install-centos7/master/usermysql.json
